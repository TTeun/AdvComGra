...
void subdivideLoop(Mesh* inputMesh, Mesh* subdivMesh) {
  ...
  // Set next and prev for the boundary edges. This is where the original method was lacking!
  int counter = 0;
  HalfEdge *next;
  // This for loop is probably not the most efficient way of doing things
  // Perhaps a nice idea is to perform an initial sort on the edges, putting the boundary ones
  // Last, then after each iteration they are still the last ones so we know where they are
  // and do not need to look through all of them. Oh well...
  for (HalfEdge &edge : subdivMesh->HalfEdges){

      if (not edge.polygon){ // Edge is a boundary edge
          ++counter;

          next = edge.twin;
          while (next->polygon) // Keep going until you find next halfedge on the boundary
              next = next->prev->twin;

          edge.next = next;   // Update 'next' for current edge
          next->prev = &edge; // Update 'prev' for next edge
      }
  }

  qDebug() << " * Set boundary " << counter << "edges";
...
QVector3D vertexPoint(unsigned short subdivType, HalfEdge* firstEdge) {
  ...
  if (isBdryVtx){ // If the vertex is on a bdry, use 1 4 6 4 1 mask along bdry. -> stencil on vtx is then [1 6 1] / 8
      vertexPt = 6 * currentEdge->prev->target->coords; // currentEdge is bdry edge pointing away from vtx
      vertexPt += currentEdge->prev->prev->target->coords;
...
QVector3D edgePoint(unsigned short subdivType, HalfEdge* firstEdge) {
  ...
  if (not firstEdge->twin->polygon){ // firstEdge is a boundary edge, use 1 4 6 4 1 mask along bdry. -> stencil on edge is then [4 4] / 8
      EdgePt = firstEdge->target->coords;
      EdgePt += firstEdge->twin->target->coords;
      EdgePt /= 2.0;
      return EdgePt;
  }
